...
class FastRouteRouter implements RouterInterface
{
     ...
     */
    final public const string CACHE_TEMPLATE
        = <<<EOT
        <?php
        return %s;
        EOT;
     ...
     */
    final public const string CONFIG_CACHE_ENABLED = 'cache_enabled';

    /**
     * @const string Configuration key used to set the cache file path
     */
    final public const string CONFIG_CACHE_FILE = 'cache_file';

    /**
     * Standard HTTP methods against which to test HEAD/OPTIONS requests.
     */
    final public const array HTTP_METHODS_STANDARD
        = [
     ...
     * FastRoute router
     */
    private readonly \FastRoute\RouteCollector $router;
    ...
    private function loadDispatchData(): void
    {
        ...
        if (!is_array($dispatchData)) {
            throw new Exception\InvalidCacheException(
                sprintf(
    ...
    private function cacheDispatchData(array $dispatchData)
    {
        ...
        if (!is_dir($cacheDir)) {
            throw new Exception\InvalidCacheDirectoryException(
                sprintf(
    ...
    {
        return fn ($data) => new GroupCountBased($data);
    }
    ...
    private function marshalMatchedRoute(array $result, string $method): RouteResult
    {
        $path = $result[1];
        $route = array_reduce(
            $this->routes, function ($matched, $route) use ($path, $method) {
                if ($matched) {
                    return $matched;
                }
    ...
    private function marshalMethodNotAllowedResult(array $result): RouteResult
    {
        ...
        $allowedMethods = array_reduce(
            $this->routes, function ($allowedMethods, $route) use ($path) {
                if ($path !== $route->getPath()) {
