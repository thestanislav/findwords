...
class WordsIterator implements Iterator
{
    ...
    public function next()
    {
        ...
        if ($this->current->nodeType == XML_TEXT_NODE || $this->current->nodeType == XML_CDATA_SECTION_NODE) {
            if ($this->offset == -1) {
                ...
                $this->words = preg_split(
                    "/[\n\r\t ]+/",
                    (string) $this->current->textContent,
                    -1,
            ...
            if ($this->offset < (is_countable($this->words) ? count($this->words) : 0)) {
                $this->key++;
                return;
            }
            $this->offset = -1;
        ...
        while ($this->current->nodeType == XML_ELEMENT_NODE && $this->current->firstChild) {
            ...
            if ($this->current->nodeType == XML_TEXT_NODE
                || $this->current->nodeType == XML_CDATA_SECTION_NODE
            ) {
                return $this->next();
            }
        }
